 ...
 *   • `db/queries/segments.ts`       — domain queries; rename + rewrite.
 *   • `agent/growthdesk.ts`          — the agent itself. Rename the file
 *                                       to match your domain, update the
 *                                       import in chat-stream/, and rewrite tools +
 *                                       instructions.
 *   • `routes/segments.ts`           — REST endpoints for the growth desk. Add
...
await createApp({
  ...
  // growthdesk.ts AgentContext + makeTools() accordingly.
  ...
  // TODO — Build 1: wire the Growth Desk routes. registerSegmentsRoutes here.
...
migrationsReady = (async () => {
  try {
    ...
    if (appConfig.data) {
      await syncFromDelta(db, appConfig.data as any);
