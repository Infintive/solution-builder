 ...
 * OpenAI-compatible interface (configured in growthdesk.ts → configureAgentsSdk).
 *
...
export async function streamAgentTurn(args: {
  ...
  modelError: import('../agent/growthdesk.js').ModelErrorDetail | null;
}> {
  ...
  const modelError: { current: import('../agent/growthdesk.js').ModelErrorDetail | null } = {
    current: null,
  };
  ...
  try {
    ...
    const agent = global.agentInstanceDEV?.agent;
    if (!agent) {
