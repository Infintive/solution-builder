...
# MAGIC - **Local (databricks-connect)** — reads `DEMO_CATALOG` / `DEMO_SCHEMA`
# MAGIC   env vars, builds a serverless `DatabricksSession`.
...
# Spark session — reuse runtime's, else build databricks-connect serverless.
try:
    spark  # noqa: F821
except NameError:
